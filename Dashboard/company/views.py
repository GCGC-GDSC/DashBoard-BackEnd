        ...
        for cmp in cmps:
            ...
            for inst in insts:
                ...
                q = CompanyCousesPlaced.objects.filter(
                    course__in=Courses.objects.filter(Q(campus=cmp) & Q(institute=inst)))
                qs = Company.objects.filter(id__in=q)
                send_data[cmp.name][inst.name].append(CompanySeralizer(qs, many=True).data)
...
class CompanyCousesRetrieveUpdateDestroy(generics.RetrieveUpdateDestroyAPIView):
